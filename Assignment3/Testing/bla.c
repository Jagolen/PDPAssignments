...
int main(int argc, char** argv){
    ...
    int extras = n%comm_sz;
    local_n = n/comm_sz;
    //if(world_rank == 0) local_n += extras;
    printf("%d local_n = %d\n", world_rank, local_n);

    int displacement[comm_sz];
	int count_send[comm_sz];
    ...
    for(int i =0; i<comm_sz; i++){
        ...
        if(i == 0){
            count_send[i] = local_n + extras;
            displacement_now += (local_n+extras);
        }
        else{
            count_send[i] = local_n;
            displacement_now += local_n;
        }
    }

    if(world_rank == 0) local_n += extras;

    if(local_n == 0) local_A = (int*) malloc(sizeof(int));
    else local_A = (int*) malloc(sizeof(int)*local_n);
    recv_buf = (int*) malloc(sizeof(int));
    merge_buf = (int*) malloc(sizeof(int));

    MPI_Scatterv(A, count_send, displacement, MPI_INT, local_A, 
    ...
    for(int iter = 0; iter < logp; iter++){
        ...
        int pair_process = (rank+(sz/2))%sz;
        int sendcount, recievecount, mergealloc;
        
        if(rank >= (sz/2)){ // in upper half
            sendcount = pivot;
            
            MPI_Sendrecv(&sendcount, 1, MPI_INT, pair_process, 1, &recievecount, 1, MPI_INT, pair_process, 1, new_comm, &status);

            if(recievecount == 0) recv_buf = (int*)realloc(recv_buf, sizeof(int));
            else recv_buf = (int*)realloc(recv_buf, recievecount*sizeof(int));
            

            mergealloc = (local_n-pivot+recievecount)*sizeof(int);
            if(mergealloc == 0) merge_buf = (int*)realloc(merge_buf, sizeof(int));
            else merge_buf = (int*)realloc(merge_buf, mergealloc);


            MPI_Sendrecv(local_A, pivot, MPI_INT, pair_process, 1, recv_buf, recievecount, MPI_INT, pair_process, 1, new_comm, &status);

            //MPI_Send(local_A,pivot,MPI_INT,pair_process,1,new_comm);    
            //MPI_Recv(recv_buf,n,MPI_INT,pair_process,1,new_comm,&status);
        }else{
            sendcount = local_n-pivot;

            MPI_Sendrecv(&sendcount, 1, MPI_INT, pair_process, 1, &recievecount, 1, MPI_INT, pair_process, 1, new_comm, &status);
            if(recievecount == 0) recv_buf = (int*)realloc(recv_buf, sizeof(int));
            else recv_buf = recv_buf = (int*)realloc(recv_buf, recievecount*sizeof(int));

            mergealloc = (pivot+recievecount)*sizeof(int);
            merge_buf = (int*)realloc(merge_buf, mergealloc);
            
            MPI_Sendrecv(local_A+pivot, local_n-pivot, MPI_INT, pair_process, 1, recv_buf, recievecount, MPI_INT, pair_process, 1, new_comm, &status);
        ...
        if(mergealloc == 0) local_A = (int*)realloc(local_A, sizeof(int));
        else local_A = (int*)realloc(local_A, mergealloc);
    ...
    if(world_rank == 0){
        local_A = (int*)realloc(local_A, n*sizeof(int));
    }
