...
/*     if(rank == 0){
    ...
    */
	...
	/* Gathering everything to output */
	printf("begin gather\n");
	printf("[%.2f...] ",l_in[0]);
	MPI_Gather( l_in , vals_per_pc , MPI_DOUBLE , output, vals_per_pc, MPI_DOUBLE, 0, cart);
	printf("end gather for rank%d\n",rank);

    if(rank==0){
		printf("total output is:\n");
